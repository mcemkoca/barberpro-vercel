/* ================================================================
   BarberPro Data Export System — export.js
   ================================================================ */
(function(){
  'use strict';

  const EX = window.BarberProExport = {};
  ...
  EX.exportPDF = function(tableSelector, filename, title, customHeaders){
    ...
    const pageTitle = title || document.querySelector('.page-title')?.textContent.trim() || 'BarberPro Report';
    const company = 'BarberPro Demo';
    ...
    doc.autoTable({
      ...
      didDrawPage: function(data) {
        ...
        doc.text('BarberPro — ' + dateStr, data.settings.margin.left, doc.internal.pageSize.height - 10);
  ...
  EX.buildButtons = function(config){
    ...
    btnExcel.onclick = () => EX.exportExcel(tableSelector, filename);
  ...
  document.addEventListener('DOMContentLoaded', function(){
    document.querySelectorAll('[data-export-table]').forEach(el => {
      ...
      const filename = el.dataset.exportFilename || 'barberpro_export';
