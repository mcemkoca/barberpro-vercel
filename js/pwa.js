...
(function() {
  ...
  const PWA = {
    ...
    lastSyncKey: 'bp_last_sync',
    installDismissedKey: 'bp_install_dismissed',
    ...
    showUpdateNotification() {
      ...
      div.id = 'bp-update-toast';
      div.style.cssText = 'position:fixed;top:80px;left:50%;transform:translateX(-50%);z-index:9999;background:var(--teal-500,#a855f7);color:white;padding:12px 24px;border-radius:var(--radius-md);font-weight:600;box-shadow:var(--shadow-lg);cursor:pointer;animation:toastIn 0.3s ease;';
    ...
    updateOfflineBadge() {
      const badge = document.getElementById('bp-offline-badge');
      if (!badge) return;
    ...
    renderSyncIndicator() {
      const indicator = document.createElement('div');
      indicator.id = 'bp-sync-indicator';
      ...
      indicator.innerHTML = `<span id="bp-sync-dot" style="width:7px;height:7px;border-radius:50%;background:var(--success-500);display:inline-block;"></span><span id="bp-sync-text"></span>`;
      document.body.appendChild(indicator);
      this.updateSyncStatus(this.isOnline ? 'synced' : 'offline');
    },

    updateSyncStatus(status) {
      const indicator = document.getElementById('bp-sync-indicator');
      const dot = document.getElementById('bp-sync-dot');
      const text = document.getElementById('bp-sync-text');
      if (!indicator || !dot || !text) return;

      const styles = {
        synced:   { color: '#a855f7', bg: 'rgba(168,85,247,0.12)', label: this._getLangText('synced', 'Senkronize') },
    ...
    triggerSync() {
      if ('sync' in navigator && this.swRegistration) {
        navigator.serviceWorker.ready.then((reg) => {
          reg.sync.register('barberpro-sync').catch(() => {});
        });
      }
      this.updateSyncStatus('syncing');
      setTimeout(() => {
    ...
    showInstallBanner() {
      if (document.getElementById('bp-install-banner')) return;
      const banner = document.createElement('div');
      banner.id = 'bp-install-banner';
      ...
      banner.innerHTML = `
        ...
        <span style="font-size:var(--text-sm);font-weight:500;color:var(--text-primary);">${this._getLangText('install_prompt', 'BarberPro\'yi ana ekranınıza ekleyin — hızlı erişim için.')}</span>
        <button id="bp-install-btn" style="background:var(--teal-500);color:white;border:none;padding:8px 18px;border-radius:var(--radius-md);font-weight:600;font-size:var(--text-sm);cursor:pointer;white-space:nowrap;">${this._getLangText('install', 'Yükle')}</button>
        <button id="bp-install-dismiss" style="background:none;border:none;color:var(--text-muted);font-size:18px;cursor:pointer;padding:4px;">✕</button>
      `;
      ...
      document.getElementById('bp-install-dismiss').addEventListener('click', () => {
      ...
      window.BPCache = {
        ...
        set: (key, data, ttlMinutes = 60) => {
          try {
            ...
            console.warn('[BPCache] Storage quota exceeded:', e);
          }
        },
        ...
        getTableData: (tableName) => BPCache.get('table_' + tableName),
        setTableData: (tableName, rows, ttl = 30) => BPCache.set('table_' + tableName, rows, ttl),
    ...
    _getLangText(key, fallback) {
      ...
      const dict = {
        tr: {
          ...
          install_prompt: 'BarberPro\'yi ana ekranınıza ekleyin — hızlı erişim için.',
          install: 'Yükle',
        ...
        en: {
          ...
          install_prompt: 'Add BarberPro to your home screen for quick access.',
          install: 'Install',
        ...
        nl: {
          ...
          install_prompt: 'Voeg BarberPro toe aan uw startscherm voor snelle toegang.',
          install: 'Installeren',
